...
//   该钩子被 opencode 同步等待），把图片 FilePart 用 doubao-seed-2.0-lite
//   （火山 Coding Plan）转成文字描述，替换为 text part。
...
const VISION_API_URL = "https://ark.cn-beijing.volces.com/api/coding/v3/chat/completions";
...
function apiKey() {
  return process.env.ARK_API_KEY || process.env.VOLC_API_KEY || "";
}
...
async function analyzeImage(dataUrl) {
  const key = apiKey();
  if (!key) throw new Error("ARK_API_KEY / VOLC_API_KEY 未设置");
...
export default async () => {
  if (!apiKey()) {
    debug("ARK_API_KEY / VOLC_API_KEY 未设置，视觉桥接不可用");
  }
  return {
