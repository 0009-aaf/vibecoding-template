...
- **视觉走查**（`docs/09-DESIGN.md` 存在时，对照逐项核对，依据 09-DESIGN 条目编号报告）：
  - 五态覆盖：本次变更涉及的数据渲染组件，§4 五态（loading/empty/error/disabled/skeleton）是否齐备
  - 令牌一致：新增样式是否只引用 §1 令牌（禁止自由发明颜色/间距/字号）
  - 断点抽查：§5 声明的关键断点行为是否满足（至少抽查一个断点）
  - 无障碍抽查：§6 基线中非"不适用"项（对比度/键盘可达/焦点可见/alt/label）抽查两项
  - 任一不符 -> 警告并列出 09-DESIGN 条目编号与实际差异
