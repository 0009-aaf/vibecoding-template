...
### 过程播报（禁止静默连跑）
- 进入每个阶段前先输出一行：`[vibe-spec] 阶段N/3：<名称>`（共 3 个阶段）
- 每阶段收尾输出一行产出物路径（如 `slices/README.md 已更新`）
...
- **并行命令清单**（利用模型高并发；AI 不自动开并行会话，供用户手动多窗口）：
  - 依赖图确定后，识别无相互依赖的切片组（可并行启动）
  - 输出每组启动命令：`/vibe-implement <编号A>` 与 `/vibe-implement <编号B>` 可同时运行（worktree + OS 切片锁保证不冲突；共享文件切片合并前必须 rebase main，见 vibe-implement 并行模式）
...
3. UI 切片必须在 spec 中引用 `docs/09-DESIGN.md` 条目编号；测试等级按切片类型取上限（全栈=UI 标准）。切片类型错标 = 后续门禁连锁失败之源。
4. 增量保护红线："已完成"切片禁止重写；重写未合并切片必须经 question 工具获用户明确选择。
5. 凡标注"用户选定/确认后再继续"处必须真实停下等待输入，禁止代答抢跑。
6. 过程播报（每阶段 ≥2 行）是硬性输出义务；完成后下一步 = `/vibe-implement <编号>`。
7. 模型档位（opencode 变体，**用户手动切换**）：契约设计与切片拆分需深度推理 → `deepseek-v4-flash-high`；默认 `deepseek-v4-flash`。