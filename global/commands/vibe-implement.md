...
- 本命令步骤多、单步耗时长，禁止静默连跑：进入每个阶段前先输出一行
  `[vibe-implement <编号>] 阶段X：<名称>`（X 取实际阶段号 0/1/2a/2b/3/3.5/3.2/4/5/6）
...
- **触发**：`--loop`/`--full` 强制执行；`--fast` 跳过（单文件改动豁免合理）
- **扫描对象**：`slices/<编号>/spec.md` 的**验收标准清单**（逐条）
- **执行步骤**：
...
4. 先写测试后写实现（红→绿）；fast 模式只减隔离成本、不减验证强度（quality-gate 必须过）。
5. 凡标注"人类验收/question 工具询问"处必须真实停下等待输入，禁止代答自验自过。
6. 过程播报（每阶段 ≥1 行 + 半行证据）是硬性输出义务；实现期样式只引用 `docs/09-DESIGN.md` §1 令牌与 §4 五态/§4.5 动效基线。
