...
## 执行锚（RECENCY ANCHOR —— 本文件任何一条与正文冲突，以本节为准；置于文首保证首屏可见）

1. 项目根 `constitution.md` 为最高约束，实现/提交违背任意条款即否决重做。
2. 编码底线：入口判空 · 精准修改 · catch 必处理 · 异常不吞 · 遇 bug 先定位根因再动手（连续 3 次修不好 = 停下上报，不蛮干）· 防御机制必测。
...
8. 模型档位（opencode 变体，**用户手动切换**，Agent 不自动切；仅在停等处建议）：深度推理（doubt 循环/契约冲突裁决）→ `deepseek-v4-flash-high`；常规实现默认 → `deepseek-v4-flash`（low）；fast/solo/机械改动 → `deepseek-v4-flash-fast`（非思考）。
...
- `--fast`: 单文件或一眼可核验 -> 轻量路径（跳过隔离，保留文档链义务）
- `--solo`: 单人临时/实验性改动 -> 最小路径（跳过隔离 + 跳过文档链义务，收尾询问补文档）
...
#### solo 最小路径（2026-08-29 开发者视角评估 P0：单人临时/实验性改动）
- **适用**：个人临时改动、实验性代码、不想开完整文档链的提交（默认问一句，不自动开）
- **不做**：切片锁/合并锁、worktree、rebase、阶段6 合并、blackboard 注册/归档、
  过程播报（仅收尾 1 行证据）、CHANGELOG/TECH-DEBT 自动登记、`docs/03-STATUS.md` 自动更新
- **仍执行（不减验证强度）**：阶段2b 写实现 -> 阶段3 全量测试 + **quality-gate 必过** + DoD 红→绿判定
- **dense-track 记录**：收尾写一行 `solo <日期> 完成层:<schema,repository,...> (solo 模式, 无新增测试, quality-gate 通过)`——中断恢复仍可定位卡点（阶段0.1 依赖）
- **测试要求（同 fast）**：solo 仅适用"无新行为"改动（文案/样式/配置/纯机械/实验性试写）。
  改动引入新行为 -> 不满足 solo 条件，回退 `--full`/`--loop`（需写测试满足 DoD 红→绿）。
- **文档链豁免的合规边界（宪法 C10 保持）**：C10"文档未同步不算完成"针对 AI 静默跳过；
  solo 不静默——收尾必须用 question 工具询问"本次改动是否需要补 CHANGELOG/STATUS 文档？"，
  由**用户显式决定**是否豁免（人类决定可覆盖文档义务），禁止 AI 自行判定"不重要不补"
- **失败/中断**：直接报告用户，无 worktree 可恢复，改动在 git 历史可见
...
### 过程播报（禁止静默连跑）
- 本命令步骤多、单步耗时长，禁止静默连跑：进入每个阶段前先输出一行
  `[vibe-implement <编号>] 阶段X：<名称>`（X 取实际阶段号 0/1/2a/2b/3/3.2/3.5/4/5/6）
- 每阶段收尾追加半行证据（测试通过数、gate 结论、截图/预览路径、commit hash 任一）
- 播报行是硬性输出义务：缺播报的阶段视为未执行；--fast 轻量路径同样适用（用 `[vibe-implement <编号>-fast]` 前缀）
- **--fast 档降级（2026-08-29 机制审计）**：fast 改动小、步骤少，不做每阶段播报——只输出 1 行收尾证据即可（`[vibe-implement <编号>-fast] 完成：<gate 结论>/<测试通过数>/<commit hash> 任一`）；full/loop 保持每阶段 ≥1 行 + 半行证据
...
> **fast/solo 模式跳过本步骤**（不建 worktree、不上锁，见"模式门控"）。以下仅 `--loop` / `--full` 执行。
...
### 阶段2a: 写测试（新增，先写测试）
> **fast/solo 模式跳过本阶段**：无新行为改动不写新测试，跑既有测试即可（见"模式门控"）。以下仅 `--loop` / `--full` 执行。
- 按 spec §测试定义，先写测试代码：
...
- **UI 切片设计前置（本切片的视觉锚点，先于写码）**：
  ...
  - **有 UI 的切片（项目在 plan 期已产出 `references/design/preview.html`）**：
    - 若 `references/design/preview.html` 已存在 → 用 playwright 打开参考（`playwright_navigate file:///...`），对照实现
    - 不存在且本切片含核心页面 → 加载 `web-artifacts-builder` 按 09-DESIGN §1 令牌兜底产出 `references/design/preview-<切片名>.html`（单文件 HTML，内联 SVG 图标）——plan 期已产出的项目此处不会触发
...
- **防御逻辑 golden-case 条款（R-02）**：本切片涉及防御/拦截/降级/守卫类逻辑（安全校验、错误兜底、降级路径、超时处理等）时，
  - 必须引用或新增 golden case（`{vault}/40_Knowledge/golden-cases/`，由 reflect.py error_pattern 分类产出）
  - 若本切片修复/新造了一个失败模式 → 在切片内记录触发条件与期望行为（事后 reflect 落盘）
  - 对应编码底线第 6 条"防御机制必测"：没有 golden case 的防御逻辑 = 未验证
...
### 阶段3.2: Converge 差距扫描（R-01，2026-08 新增）
> 来源：GitHub Spec Kit 1.0 `converge` + Vidocq 实战教训（"AI 会静默漂移，需要循环收敛"）。
> 目的：实现+测试通过 ≠ spec 兑现——自动对照验收标准找差距，缺什么补什么，直到收敛才放行。

- **触发**：`--loop`/`--full` 强制执行；`--fast`/`--solo` 跳过（单文件/临时改动豁免合理）
- **扫描对象**：`slices/<编号>-<名称>/spec.md` 的**验收标准清单**（逐条）
- **执行步骤**：
  1. 逐条验收标准 -> 找**代码证据**（对应测试文件 + 测试通过输出 / 相关实现文件路径）
  2. 产出 `.vibecoding/converge-<编号>.md` 差距清单，三态逐条列出：
     - `✓ 已满足` + 证据路径（测试文件/命令输出）
     - `✗ 未满足` + 缺什么（无测试 / 无实现 / 行为不符）
     - `⏸ 豁免` + 理由（必须登记到 `docs/TECH-DEBT.md`）
  3. **回环规则**：`✗ 未满足` > 0 -> 回到阶段2a/2b 补实现/补测试 -> 重跑阶段3 -> 重扫 Converge
     （每回环一次，`slices/README.md` 该切片"converge 轮次"列 +1）
  4. **豁免上限**：`⏸ 豁免` 项数 ≤ 验收标准总数 20%；超限 -> 阻断，禁止进阶段4
  5. **收敛条件**：`✗` = 0 且 豁免 ≤ 20% -> 差距清单随切片提交（git 追踪），进入阶段 3.5
- **反合理化**：
  | 借口 | 拆穿 |
  |---|---|
  | "测试过了就是满足了" | 测试通过 ≠ 覆盖验收标准：Converge 按 spec 逐条对账，不是看测试绿色 |
  | "这条验收标准不重要" | 不重要当初就不该写进 spec；现在豁免必须登记技术债 + 20% 上限 |
  | "回环浪费时间" | 一次回环 < 合并后返工；audit 发现差距 = 整片打回更贵 |
...
### 阶段6: 原子化合并（重构）
> **fast/solo 模式跳过本阶段**：无 worktree/分支/锁，验收通过后直接在主工作区 `git commit`
> （见"模式门控"），然后更新 `docs/03-STATUS.md` 为已完成并清 blackboard SID
> （`python ~/.claude/harness/blackboard.py archive <SID>`；solo 未注册 blackboard，仅问补文档）。
> 以下仅 `--loop` / `--full` 执行。

#### 6.0 提交消息（CM11：文件功能清单，两种模式都适用）
- 格式：`<type>(<scope>): <名词性功能描述>`（CM1-CM3；subject 说明涉及对象及其职责，不用"新增/修复/改为"类变更动词）+ Body 文件功能清单（CM11）
- Body 每个变更文件一行（全部文件都列）：`<文件>: <该文件的功能>`
  例：`src/auth/login.ts: 登录页表单——账号密码提交与前端校验`
- 只描述文件本身是干什么的，不描述本次改了什么（改动内容看 diff）
- 快速自检：不读 diff、只读 message，能说清"每个涉及的文件是干什么的"才算合格
