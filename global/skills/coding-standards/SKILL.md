...
| CM11 | Body 含文件功能清单：每个变更文件一行（全部文件都列，不挑"非琐碎"），描述文件本身是干什么的（例：`quality-gate.js`: 提交前质量门禁——密钥扫描、测试强制、Protected Region 校验），不描述本次改了什么（改动内容看 diff）；subject 保留 type 前缀、用名词性功能描述（说明涉及对象及其职责），不用"新增/修复/改为"等变更动词；让读提交历史的人不查目录就知道涉及文件各自的职责 |
